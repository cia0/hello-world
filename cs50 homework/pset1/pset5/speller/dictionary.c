...
int pie;
int dictsize = 0; //count the size of the dictionary as it loads
...
{
    root = calloc(1,sizeof(node)); //root array node
    ...
    for (int c; (c = fgetc(fpd)) != EOF;)
    {
        ...
        if(beggining) //beggining of a word
        {
            ...
            if (root->child[vine] == NULL)
            {
                ...
                root->child[vine] = calloc(1,sizeof(node));
        ...
        else if(c == '\n') //end of line
        {
            beggining = true;
            trav->is_word = true;
            dictsize++; //increments the counted size of the dictionary
            //printf("\n");
            continue;
        }
        ...
        else //in the middle of a word
        {
            ...
            {
                trav->child[vine] = calloc(1,sizeof(node)); //creates a new node if at dead end
            }
